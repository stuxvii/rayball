...
async fn main() -> Result<(), Error> {
    ...
    clr_val!(PRIMARY_COLOR) = get_gui_color(rl.gui_get_style(GuiControl::DEFAULT, GuiControlProperty::TEXT_COLOR_NORMAL));
    ...
    while !rl.window_should_close() {
        ...
        if !in_game {
            ...
            if time_timer > 0.5 {
                let mut fmt = "%I:%M:%S";
                if cfg_val!(atomget MILITARY_TIME) {fmt = "%H:%M:%S";}
                time_txt = Local::now().format(fmt).to_string();
                time_timer = 0.;
            }

            d.draw_text(&time_txt, (screen_width / 2) - (text_widths.get("clck").unwrap() / 2), layout::SPACING as i32 * 2, layout::FONT_SIZE, clr_val!(PRIMARY_COLOR));
